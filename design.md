...
**`api:` parameter — inline API annotation.** `DSField` accepts an optional `api:` string: a `.dsCodeMicro` tertiary mono line rendered DIRECTLY UNDER the label (always its own wrapping line — never truncated, never sharing the label line; the inspector pane is narrow by nature and a clipped annotation teaches nothing) naming the exact official Apple API symbol/argument the control feeds (e.g. `"NLTokenizer.setLanguage : NLLanguage"`, `"@Generable struct Name"`). These strings come exclusively from `Core/APICatalog.swift` structured data (`APIArgument.param` / `.fromInput`) — never hand-written prose. Keep captions short (a symbol, not a sentence); long explanations belong in `help:`, not `api:`. Ops with a deterministic output shape also surface a concrete "Example output" block (`HookOp.outputPreview(projection:)`) — the output of a node is never a black box.
