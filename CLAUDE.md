...
- `Core/APICatalog.swift` — node/op → official Apple API registry (symbol · per-argument GUI mapping · doc link); drives the searchable op picker, every node's "API mapping" inspector section, and `GraphNode.apiName`. Adding a native op = one `HookOp` case + one catalog entry + one `HookEngine.apply` branch.
