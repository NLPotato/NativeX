...
- `Core/TranscriptDef.swift` — Codable mirror of `Transcript`; THE conversation-lane protocol (group emits the request → FM seeds the session, responds, reads `session.transcript` back → `transcript` port + trace). See `docs/reference/foundation-models.md` §Transcript.
- `Core/APICatalog.swift` — node/op → official Apple API registry (symbol · per-argument GUI mapping · doc link); drives the searchable op picker, every node's "API mapping" inspector section, and `GraphNode.apiName`. Adding a native op = one `HookOp` case (with `returnShape` + `paramKeys`) + one catalog entry + one `HookEngine.apply` branch + the op's name in `NodeInspector`'s per-node-kind `candidates` list (hardcoded, not catalog-derived — the one spot the build can't catch). Inspector *fields* are declaration-driven; version-gated ops set `since`/`fallback` on the catalog entry so the picker doesn't over-claim.
...
- `Models/Storage.swift` — SwiftData models: `PromptTemplateModel` + `SchemaModel` (both versioned), `DatasetModel`→`ExampleModel`, `ExperimentModel`→`RunModel`, `GraphModel`, `TraceModel` (run history), `HookScriptModel` (saved hook scripts), + `RunInput` (built-in tasks define their own `Gloss.Input`/`Roleplay.Input` in `Tasks/`); plus `JSONCoder`. Schemas save/load (versioned) from the Guided-output inspector; scripts from the Hook inspector.
